 ...
 * Transport : stdio (dev local) ou HTTP via StreamableHTTPServerTransport (Docker prod).
 */
...
// Dispatcher les appels de tools
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    return {
      content: [{ type: 'text' as const, text: `Erreur inattendue dans ${name} : ${msg}` }],
      isError: true,
    };
  }
});
